...
export class CosasFeasPage implements OnDestroy {
  ...
  createChart(labels: string[], data: number[]) {
    ...
    const options: any = {
      ...
      plugins: {
        legend: {
          display: tipo === 'bar' // Ocultar leyenda para gráficos de torta
        }
      },
  ...
  async loadImages() {
    ...
    const imagesCollection = collection(this.firestore, `imagenes-feas`);
    ...
    for (const imageDoc of snapshotTodas.docs) {
      ...
      const likeDocRef = doc(this.firestore, `likes-feas/${this.userId}_${imageDoc.id}`);
  ...
  async toggleLike(image: any) {
    ...
    const likeDocRef = doc(this.firestore, `likes-feas/${this.userId}_${image.id}`);

    const imageDocRef = doc(this.firestore, `imagenes-feas/${image.id}`);
    ...
    if(!image.liked){
      ...
      const likesQuery = query(
        collection(this.firestore, `likes-feas`),
